...
/**
 * The design's CONVERSATION section — one switch, and it does something.
 *
 * **Pin to top** sorts the thread first in your list, in the server's own ORDER BY, and for
 * you only. Nobody else can tell.
 *
 * ## Why there is no "Mute notifications"
 *
 * The reference draws one, and StarLink deliberately does not have it. A notification is how
 * the product tells somebody a conversation needs them, and a switch that turns that off for
 * one thread is a switch for missing the thread that mattered — usually the busy one it felt
 * reasonable to mute. §29.6 already refuses to make the in-app unread count a preference;
 * this is the same reasoning one layer out, and it was taken as a product decision rather
 * than inherited from the mock. The column is gone too (migration 0018), so bringing mute
 * back is a schema change and not an afternoon.
 *
 * What a person CAN do instead is on the settings screen: quiet hours, which silence this
 * device for everything equally and cannot single out one conversation.
 *
 * Optimistic, and reverted on failure. A switch that waits for a round trip before moving
 * reads as broken; one that moves and stays moved after a refusal is a lie.
 */
export function ConversationControls({
  ...
  /** The list is sorted by `pinned` server-side, so it has to be re-read when this moves. */
  readonly onChanged: () => void;
}): React.JSX.Element {
  const [state, setState] = useState(pinned);
  const [problem, setProblem] = useState<string | undefined>();

  /* The summary is the source of truth; this follows it when the shell re-reads, so two
     tabs cannot disagree about a switch either of them moved. */
  useEffect(() => setState(pinned), [pinned]);
  ...
  const set = async (next: boolean): Promise<void> => {
    ...
    try {
      await api.setConversationPreferences(conversationId, { pinned: next });
      onChanged();
    } catch {
      setState(previous);
  ...
  return (
    <section className="details-section">
      <h3 className="details-section-title">Conversation</h3>

      <div className="details-switches">
        <Switch label="Pin to top" checked={state} onToggle={() => void set(!state)} />
      </div>
...
/** The design's 40x24 switch: label left, track right, no third state. */
function Switch({
  label,
  checked,
  onToggle,
